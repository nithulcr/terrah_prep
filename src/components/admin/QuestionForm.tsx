...
const emptyForm: FormValues = {
  batch_id: '', category_id: '', question_text: '', option_a: '', option_b: '', option_c: '', option_d: '',
  correct_option: 'A', explanation: '', difficulty: 'easy', language: 'english', marks: 1,
  negative_marks: 0, question_image: '', is_active: true,
};

export function QuestionForm({ question }: { question?: Question }) {
  const router = useRouter();
  const [values, setValues] = useState<FormValues>(question ? {
    batch_id: question.batch_id, category_id: question.category_id, question_text: question.question_text,
    ...
    question_image: question.question_image ?? '', is_active: question.is_active,
  } : emptyForm);
  ...
  const submit = async (event: FormEvent) => {
    ...
    const payload = { ...values, explanation: values.explanation || null, question_image: values.question_image || null };
  ...
  return <form onSubmit={submit} className="space-y-6">
    <Card><CardBody className="space-y-4 p-6">
      <div className="grid gap-4 md:grid-cols-2">
        <label>Batch<select required value={values.batch_id} onChange={(e) => set('batch_id', e.target.value)} className="mt-1 w-full rounded border p-2"><option value="">Select batch</option>{batches.map((batch) => <option key={batch.id} value={batch.id}>{batch.batch_name}</option>)}</select></label>
        <label>Category<select required value={values.category_id} onChange={(e) => set('category_id', e.target.value)} className="mt-1 w-full rounded border p-2"><option value="">Select category</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>
      </div>
      <label className="block">Question<Textarea required value={values.question_text} onChange={(e) => set('question_text', e.target.value)} rows={4} /></label>
