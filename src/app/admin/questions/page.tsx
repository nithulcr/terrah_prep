...
export default function QuestionsPage() {
  ...
  useEffect(() => { void load(); }, []);
  const remove = async (id: number) => { if (!confirm('Delete this question?')) return; const { error } = await supabase.from('questions').delete().eq('id', id); if (error) return alert(error.message); setQuestions((all) => all.filter((question) => question.id !== id)); };
