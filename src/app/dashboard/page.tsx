...
export default function DashboardPage() {
  const { profile, subscription, usage, loading, user } = useAuth();
  
  try {
    // Safe defaults for all values - NEVER crash
    const planName = subscription?.plan?.name ?? 'Free';
    const displayName = profile?.role === 'admin' ? 'Admin' : profile?.full_name?.split(' ')[0] || 'Student';

    console.log('Dashboard: Render - loading:', loading, 'user:', !!user, 'profile:', !!profile, 'role:', profile?.role);
    ...
    // If not authenticated, this shouldn't render (middleware should catch it)
    ...
    // Dashboard is accessible to all authenticated users (admin and regular users)
    // Always render dashboard, even if profile/subscription is still loading
    return (
    ...
    return (
      <main className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          <p className="text-slate-600">Error loading dashboard. Please refresh the page.</p>
          <button 
